...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!q) {
      return NextResponse.json({ success: false, error: '請輸入搜尋條件' }, { status: 400 });
    }

    // Search for member by phone or name
    const memberResult = await query(`
      ...
      WHERE phone LIKE $1 OR name ILIKE $2
      LIMIT 1
    `, [`%${q}%`, `%${q}%`]);
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        line_bindind: member.line_binding,
