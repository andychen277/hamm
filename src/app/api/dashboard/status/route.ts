...
export async function GET() {
  try {
    const [repairs, orders, watchlistPending] = await Promise.all([
      // Active repairs
      query<{ count: string }>(
      ...
      // Watchlist awaiting notification
      query<{ count: string }>(
        `SELECT COUNT(*) as count FROM watchlist WHERE notified = false`
      ),
    ]);
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        watchlist_pending: Number(watchlistPending.rows[0].count),
      },
    });
  } catch (error) {
    return NextResponse.json(
